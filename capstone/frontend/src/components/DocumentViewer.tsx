...
export const DocumentViewer: React.FC<DocumentViewerProps> = ({ document, onError }) => {
            ...
            {structure && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-gray-900">{structure.total_chunks}</p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg">
                ...
                {getFilteredChunks().map((chunk, index) => (
                    <div key={chunk.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center space-x-2">
                                <span className={`px-2 py-1 rounded text-xs font-medium ${getChunkTypeColor(chunk.chunk_type)}`}>
                                    {chunk.chunk_type}
                                </span>
                        ...
                        {chunk.metadata && Object.keys(chunk.metadata).length > 0 && (
                            <div className="mt-2 pt-2 border-t border-gray-100">
                                <div className="text-xs text-gray-500">
                                    Words: {chunk.metadata.word_count || 'N/A'} • 
                                    Characters: {chunk.metadata.char_count || 'N/A'}
                                </div>
                            </div>
                        )}
                    </div>
